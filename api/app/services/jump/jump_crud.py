...
def create_jump_data_frame(file, filename, user_id):
        dataset = DatasetService(file.file, filename, user_id)
...
class JumpCRUD(InfluxDBService):
    ...
    def create_jump(self, user_id: UUID, file: UploadFile):
        df, dataset_name = create_jump_data_frame(file, file.filename, user_id)
        db_jump = JumpCreate(id=uuid4(), name=dataset_name, user_id=user_id, data=df.to_json(default_handler=str, orient='records'))
